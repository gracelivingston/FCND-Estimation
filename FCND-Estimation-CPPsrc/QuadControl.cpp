    ...
    float collThrustCmd)
{
  ...
  // HINTS:
  //  - we already provide rotation matrix R:
  //    to get element R[1,2] (python) use R(1,2) (C++)
  //  - you'll need the roll/pitch gain kpBank
  //  - collThrustCmd is a force in Newtons! You'll
  //    likely want to convert it to acceleration first
    ...
    float accelZCmd, float dt)
{
  ...
  // vertical cmd + position gain X position error
  velZCmd += kpPosZ * dp; // p + d

  // vertical cmd - vertical speed
  float dv =
      CONSTRAIN(velZCmd, -maxAscentRate, maxDescentRate) - velZ;

  // vertical acceleration + position gain X S dp dt
  float u = accelZCmd + kpVelZ * dv + KiPosZ * integratedAltitudeError;

  // Translate force to body frame and compensate for gravity
    ...
    V3F accelCmdFF)
{
  ...
  // constrain to max horizontal velocity
