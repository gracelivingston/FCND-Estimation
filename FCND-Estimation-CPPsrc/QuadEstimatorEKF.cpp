...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
    ...
    // Integrate the current state over dt to find the new predicted state.
    ...
    // Update the predicted state with the current acceleration.
    predictedState(3) = curState(3) + dt * a_I.x;
    predictedState(4) = curState(4) + dt * a_I.y;
    predictedState(5) = curState(5) + dt * a_I.z - dt * (float) CONST_GRAVITY;
